...
from skill_mcp.models import FileInfo, ScriptInfo, SkillDetails, SkillMetadata, SkillSummary
    ...
    def get_skill_details(skill_name: str) -> SkillDetails:
        ...
        if skill_md_path.exists():
            try:
                ...
                if parsed:
                    ...
                    metadata.extra = {
                        k: v for k, v in parsed.items() if k not in ("name", "description")
                    }
        ...
        for file_info in files_list:
            ...
            if is_exec:
                scripts.append(
                    ScriptInfo(
                        path=file_info["path"], type=file_type, has_uv_deps=has_uv_deps or False
                    )
                )
